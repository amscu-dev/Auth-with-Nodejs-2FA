...
export const passkeySignUpInitMutationFnBody = zod.object({
  "name": zod.string().describe('The display name of the user registering a passkey.'),
  "email": zod.string().describe('The email address of the user registering a passkey.')
})
...
export const passkeySignUpVerifyMutationFnBody = zod.object({
  "id": zod.string().describe('Base64Url-encoded credential ID generated by the authenticator.'),
  "rawId": zod.string().describe('Raw Base64Url-encoded credential ID.'),
  "type": zod.enum(['public-key']).describe('Must always be \"public-key\".'),
  "response": zod.object({
  "attestationObject": zod.string().describe('Base64Url-encoded attestation object provided by the authenticator.'),
  "clientDataJSON": zod.string().describe('Base64Url-encoded JSON object with client data.'),
  "authenticatorData": zod.string().optional().describe('Optional authenticator data returned by the device.'),
  "publicKey": zod.string().optional().describe('Optional public key generated by the authenticator.'),
  "publicKeyAlgorithm": zod.number().optional().describe('Optional algorithm identifier for the public key.'),
  "transports": zod.array(zod.enum(['ble', 'cable', 'hybrid', 'internal', 'nfc', 'smart-card', 'usb'])).optional().describe('Optional transport methods supported by the authenticator.')
}),
  "authenticatorAttachment": zod.enum(['cross-platform', 'platform']).optional().describe('Optional attachment modality of the authenticator.'),
  "clientExtensionResults": zod.object({
  "appid": zod.boolean().optional().describe('Indicates if the \"appid\" extension was used.'),
  "credProps": zod.object({
  "rk": zod.boolean().optional().describe('Indicates if the credential is resident.')
}).optional().describe('Credential properties returned by the client.'),
  "hmacCreateSecret": zod.boolean().optional().describe('Indicates if the \"hmacCreateSecret\" extension was used.')
}).optional().describe('Optional results of any WebAuthn client extensions.')
})
 ...
 */
export const passkeySignInVerifyMutationFnBody = zod.object({
  "id": zod.string().describe('The base64url-encoded credential ID uniquely identifying the passkey credential.\n'),
  "rawId": zod.string().describe('The raw credential ID in base64url format. Usually identical to `id` but encoded differently.\n'),
  "type": zod.enum(['public-key']).describe('The credential type. For WebAuthn, always `\"public-key\"`.'),
  "authenticatorAttachment": zod.enum(['cross-platform', 'platform']).optional().describe('Indicates the attachment modality of the authenticator:\n- `\"platform\"`: built into the device (e.g. Touch ID, Windows Hello)\n- `\"cross-platform\"`: external authenticator (e.g. security key)\n'),
  "clientExtensionResults": zod.object({
  "appid": zod.boolean().optional().describe('Indicates if the AppID extension was used.'),
  "credProps": zod.object({
  "rk": zod.boolean().optional().describe('Whether the credential is a resident key.')
}).optional().describe('Credential properties returned by the authenticator.'),
  "hmacCreateSecret": zod.boolean().optional().describe('Indicates if the `hmac-secret` extension was used.')
}).optional().describe('Results from any WebAuthn extensions used during authentication.'),
  "response": zod.object({
  "clientDataJSON": zod.string().describe('Base64url-encoded JSON object that was passed to the authenticator.'),
  "authenticatorData": zod.string().describe('Base64url-encoded authenticator data containing RP ID hash, flags, and signature counter.'),
  "signature": zod.string().describe('Base64url-encoded signature that proves possession of the private key.'),
  "userHandle": zod.string().describe('Base64url-encoded identifier for the user account associated with the credential.')
}).describe('Contains the raw cryptographic materials returned by the authenticator, required for verifying the authentication response.\n')
}).describe('Represents the **WebAuthn authentication response** sent by the browser after calling `navigator.credentials.get()` during the passkey sign-in process. This object contains all cryptographic data needed by the backend to verify the authenticity and integrity of the authentication ceremony.\n')

export const passkeySignInVerifyMutationFnResponse = zod.union([zod.object({
  "success": zod.literal(true),
...
export const passkeyAddPasskeyByUserIdInitMutationFnParams = zod.object({
  "userid": zod.string().describe('The unique identifier of the user who is adding a new passkey. Must match the currently authenticated user.')
})
...
export const passkeyRemovePasskeyByIdInitMutationFnParams = zod.object({
  "userid": zod.string().describe('The unique identifier of the user who is removing a passkey.   This must match the authenticated user\'s ID. Only the user who owns the passkey can initiate the removal.\n'),
  "credentialid": zod.string().describe('The unique identifier of the passkey credential that is being removed.   This should match the credential ID of the passkey to be deleted.\n')
})
...
export const passkeyRemovePasskeyByIdVerifyMutationFnBody = zod.object({
  "id": zod.string().describe('The base64url-encoded credential ID uniquely identifying the passkey credential.\n'),
  "rawId": zod.string().describe('The raw credential ID in base64url format. Usually identical to `id` but encoded differently.\n'),
  "type": zod.enum(['public-key']).describe('The credential type. For WebAuthn, always `\"public-key\"`.'),
  "authenticatorAttachment": zod.enum(['cross-platform', 'platform']).optional().describe('Indicates the attachment modality of the authenticator:\n- `\"platform\"`: built into the device (e.g. Touch ID, Windows Hello)\n- `\"cross-platform\"`: external authenticator (e.g. security key)\n'),
...
export const passkeyAllGetByUserIdQueryFnParams = zod.object({
  "userid": zod.string().describe('The unique identifier of the user for whom the sessions are being retrieved.')
})

export const passkeyAllGetByUserIdQueryFnResponse = zod.object({
  "success": zod.boolean().optional().describe('Indicates whether the request was successful.'),
  "statusCode": zod.number().optional().describe('HTTP status code.'),
  "message": zod.string().optional().describe('A message summarizing the request result.'),
  "data": zod.object({
  "passkeys": zod.array(zod.object({
  "aaguid": zod.object({
  "name": zod.string().optional().describe('The name of the passkey provider.'),
  "icon_dark": zod.string().optional().describe('The base64 encoded SVG for the dark theme icon.'),
  "icon_light": zod.string().optional().describe('The base64 encoded SVG for the light theme icon.')
}).optional(),
  "credentialId": zod.string().optional().describe('The unique identifier of the passkey credential.'),
  "createdAt": zod.iso.datetime({}).optional().describe('The date and time when the passkey was created.'),
  "lastUsed": zod.iso.datetime({}).optional().describe('The date and time when the passkey was last used.')
})).optional()
}).optional(),
  "metadata": zod.object({
  "requestId": zod.string().optional().describe('A unique identifier for the request.'),
  "count": zod.number().optional().describe('The total number of passkeys retrieved.')
}).optional()
})
